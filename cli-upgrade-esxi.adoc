...
summary: 如果您在 VMware ESXi 上執行ONTAP Select ，則可以將 ESXi 軟體從早期支援的版本升級到 ESXi 8.0 或更高版本。在升級之前，您應該了解升級流程並選擇合適的升級步驟。 
---
= 將ONTAP Select升級到 VMware ESXi 8.0 或更高版本
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
如果您在 VMware ESXi 上執行ONTAP Select ，則可以將 ESXi 軟體從早期支援的版本升級到 ESXi 8.0 或更高版本。在升級之前，您應該了解升級流程並選擇合適的升級步驟。
...
+
使用 Deploy 管理公用程式執行 ESXi 升級是更通用且更具彈性的選項。但是，有時可能會出現部署不可用或無法使用的情況。例如，早期版本的ONTAP Select和 Deploy 管理實用程式不支援升級到 ESXi 8.0。
...
----
. 使用 VMware 提供的步驟將執行ONTAP Select 的虛擬機器管理程式主機升級到 ESXi 8.0 或更高版本。
. 將節點移至線上狀態：
+
[source, cli]
----
