...
NOTE: 與內部流量類似，外部流量可以在 vSwitch 層（VST）和外部交換器層（EST）進行標記。此外，ONTAP Select VM 本身也可以透過稱為 VGT 的程序對外部流量進行標記。如需更多詳細資訊，請參閱 link:concept_nw_data_mgmt_separation.html["資料與管理流量分離"] 章節。
