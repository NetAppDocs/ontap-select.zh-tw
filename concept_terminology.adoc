...
叢集ONTAP Select:: 您可以建立一個由一個、兩個、四個、六個、八個、十個或十二個節點組成的_ONTAP Select叢集_。多節點叢集始終包含一個或多個 HA 對。例如，一個四節點群集由兩個 HA 對組成。單節點叢集不提供 HA 功能。
...
如需更多資訊，請參閱 link:concept_workflow_deploy.html["規劃"] 和 link:concept_lic_evaluation.html["授權"] 這兩個部分。
...
認證儲存區:: Deploy 憑證存放區是一個安全的資料庫，用於保存帳戶憑證。它主要用於在建立新叢集時註冊 Hypervisor 主機。如需詳細資訊，請參閱 link:concept_workflow_deploy.html["規劃"] 章節。
...
+
--
vNAS 的預設設定啟用了一項名為單一實例資料日誌記錄（SIDL）的寫入最佳化功能。在 ONTAP Select 9.6 及更高版本中，啟用 SIDL 後，背景 ONTAP 儲存效率功能將會進一步提升。如需更多資訊，請參閱 link:concept_stor_concepts_chars.html["深入探討"] 章節。
