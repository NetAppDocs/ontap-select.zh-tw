...
不支援在 Deploy 執行個體於環境中執行後修改其網路詳細資訊。如需詳細資訊，請參閱知識庫文章 link:https://kb.netapp.com/onprem/ontap/ONTAP_Select/Modifying_DNS_configuration_of_ONTAP_Deploy_instance["修改ONTAP Deploy 實例的 DNS 配置"^]。
...
如果您需要恢復 ONTAP Select 節點，例如 ONTAP Select 虛擬機器或主機完全遺失，或者單節點叢集被意外刪除，請聯絡 NetApp 技術支援。如需更多資訊，請參閱知識庫文章 link:https://kb.netapp.com/on-prem/ontap/ontap-select/Select-KBs/ONTAP_Select_node_VM_recovery_in_case_of_disaster["災難發生時ONTAP Select節點虛擬機器恢復"^]。
