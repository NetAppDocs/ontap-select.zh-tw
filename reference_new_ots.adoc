...
--
支援 SnapMirror Cloud:: 從 ONTAP Select 9.17.1 開始，ONTAP Select 支援 SnapMirror Cloud。SnapMirror Cloud 是 ONTAP 授權功能，當您部署 ONTAP Select 9.17.1 叢集或將現有 ONTAP Select 叢集升級到 9.17.1 版本時，預設會啟用此功能。
+
--
https://docs.netapp.com/us-en/ontap/concepts/snapmirror-cloud-backups-object-store-concept.html["了解 SnapMirror Cloud 備份到物件儲存設備"^]。
...
--
已更新 KVM Hypervisor 支援:: 從 ONTAP Select 9.17.1 開始，Red Hat Enterprise Linux (RHEL) 10.1、10.0、9.7 和 9.6 以及 Rocky Linux 10.1、10.0、9.7 和 9.6 支援 KVM 虛擬機器管理程式。
+
--
[NOTE]
====
在 RHEL 10.1 和 10.0 以及 Rocky Linux 10.1 和 10.0 主機上，KVM 虛擬機器管理程式的軟體 RAID 工作流程有一些限制。如需更多資訊，請參閱以下知識庫文章：
