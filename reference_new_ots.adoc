...
支援外部金鑰管理 (KMIP):: 從 ONTAP Select 9.19.1 開始，ONTAP Select 支援使用金鑰管理互通協定（KMIP）進行外部金鑰管理。您可以在部署 ONTAP Select 9.19.1 叢集或將現有叢集升級到 ONTAP Select 9.19.1 後啟用外部金鑰管理。如需更多資訊，請參閱 link:reference_lic_ontap_features.html#licensed-ontap-features["預設授權的 ONTAP 功能"]。
+
--
ONTAP Select 不支援雲端金鑰管理程式。

--
支援 Advanced Micro Devices (AMD) 處理器:: 從 ONTAP Select 9.19.1 開始，ONTAP Select 支援在 Kernel-Based Virtual Machine（KVM）和 ESXi Hypervisor 主機上使用 AMD 處理器。如需詳細資訊，請參閱link:reference_plan_ots_hardware.html["VMware 虛擬機器管理程式與硬體注意事項"]。
...
| ONTAP Select | 部署ONTAP Select | link:task_install_deploy.html["安裝ONTAP Select 部署"] 


| ONTAP Select 映像安裝 | ONTAP Select 映像 | link:task_cli_deploy_image_add.html["新增要部署的 ONTAP Select 映像"] 


| ONTAP Select Deploy 升級 | 部署ONTAP Select | link:task_cli_upgrade_deploy.html["升級 ONTAP Select 部署執行個體"] 


| ONTAP Select 節點升級 | ONTAP Select 映像 | link:concept_adm_upgrading_nodes.html["升級 ONTAP Select 節點"] 
|===
--
增強對集群擴展和收縮的支持:: 從ONTAP Select 9.18.1 開始，支援在 ESXi 和 KVM 虛擬機器管理程式主機上，四節點和十二節點ONTAP Select叢集之間進行叢集擴展和收縮。
+
--
--
...
--
已更新 KVM Hypervisor 支援:: 從 ONTAP Select 9.18.1 開始，Rocky Linux 10.1 支援 KVM Hypervisor。
...
KVM 主機上本機連接的 NVMe 磁碟的軟體 RAID 支持:: ONTAP Select 9.17.1 為 KVM 虛擬機器管理程式主機上的 ONTAP Select 本機連接 NVMe 磁碟引進軟體 RAID 支援。
+
--
...
--
支援 SnapMirror 雲端:: 從 ONTAP Select 9.17.1 開始，ONTAP Select 支援 SnapMirror cloud。ONTAP Select 預設不提供 SnapMirror cloud 授權。部署 ONTAP Select 9.17.1 叢集或將現有 ONTAP Select 叢集升級至 9.17.1 版本時，您需要申請 SnapMirror cloud 授權。如需更多資訊，請參閱link:reference_lic_ontap_features.html#other-ontap-features["個別授權的功能ONTAP"]。
支援 SnapLock Select:: 從 ONTAP Select 9.17.1 開始，新部署會自動授權 SnapLock Select（包括防篡改 Snapshot 鎖定）。若要從 ONTAP Select 9.16.1 及更早版本升級，您可以從link:https://mysupport.netapp.com/site/["NetApp 支援網站"^]免費下載 SnapLock Select 授權並手動套用。如需更多資訊，請參閱link:reference_lic_ontap_features.html#licensed-ontap-features["預設授權的 ONTAP 功能"]。
...
[NOTE]
====
若要執行內容庫操作，您需要存取額外的權限等級。如果您在 vSAN ESA 部署中使用受限角色，則必須擁有此存取權限。

link:reference_plan_dep_vmware.html#vmware-vcenter-server-considerations["了解內容庫操作的存取權限級別"].
...
--
已更新 KVM Hypervisor 支援:: 從 ONTAP Select 9.17.1 開始，Red Hat Enterprise Linux（RHEL）10.1、10.0、9.7 和 9.6 以及 Rocky Linux 10.0、9.7 和 9.6 支援 KVM 虛擬機器管理程式。
+
--
[NOTE]
====
...
====
--
增強VMware ESXi支援:: ONTAP Select 9.17.1 包含對 VMware ESXi 9.0 的支援。
...
更新的 NetApp 授權檔案支援:: 從 ONTAP Select 9.16.1 開始，NetApp 授權檔案（NLF）支援已更新。新的 NLF 格式包含 ARP、ONTAP S3 和 S3 SnapMirror 功能的授權。link:reference_lic_ontap_features.html#licensed-ontap-features["深入瞭解"]。
+
--
...
--
支援自主勒索軟體保護:: ONTAP Select 9.16.1 引入了對自主勒索軟體防護（ARP）的支援。ONTAP Select 9.16.1 僅支援 ARP 的手動更新，不支援自動更新。ARP 功能授權包含在 ONTAP Select 9.16.1 的 NLF 中。link:reference_lic_ontap_features.html#licensed-ontap-features["深入瞭解"]。
...
更新的 ONTAP Select 部署支援:: 如果您目前執行的 ONTAP Select Deploy 9.14.1 版本低於 9.14.1P2，則應盡快升級至 ONTAP Select Deploy 9.14.1P2。有關更多資訊，請參閱 link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["ONTAP Select 9.14.1 版本資訊"^]。
...
更新的 ONTAP Select 部署支援:: 截至 2024 年 4 月，ONTAP Select Deploy 9.13.1 已不再在 NetApp 支援網站上提供。如果您正在執行 ONTAP Select Deploy 9.13.1,則應盡快升級至 ONTAP Select Deploy 9.14.1P2。如需詳細資訊,請參閱 link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["ONTAP Select 9.14.1 版本資訊"^]。
...
截至 2024 年 4 月,ONTAP Select Deploy 9.12.1 已不再在 NetApp 支援網站上提供。如果您正在執行 ONTAP Select Deploy 9.12.1,則應盡快升級至 ONTAP Select Deploy 9.14.1P2。如需詳細資訊,請參閱 link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2886733["ONTAP Select 9.14.1 版本資訊"^]。
...
+
NetApp 強烈建議客戶規劃並執行從 ONTAP Select for KVM 到任何其他 ONTAP 平台（包括 ONTAP Select for ESXi）的完整資料遷移。如需更多資訊，請參閱 https://mysupport.netapp.com/info/communications/ECMLP2877451.html["EOA通知"^]
