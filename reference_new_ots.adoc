...
叢集擴充:: ONTAP Select 9.13.1 引進叢集擴充功能、可提供高可用度（ HA ）保護、避免節點中斷對叢集服務資料的能力造成影響。從 ONTAP Select 9.13.1 開始、您可以將兩個節點設定為 HA 配對、將單節點叢集擴充至雙節點叢集。此程序是透過 CLI 或 Web 介面、從 ONTAP Select 部署中自動啟動。若要執行此程序、您必須符合特定的硬體和軟體需求。 link:concept_cluster_expansion.html["深入瞭解"]。
