...
支援外部金鑰管理 (KMIP):: 從 ONTAP Select 9.19.1 開始，ONTAP Select 支援使用金鑰管理互通協定（KMIP）進行外部金鑰管理。您可以在部署 ONTAP Select 9.19.1 叢集或將現有叢集升級到 ONTAP Select 9.19.1 後啟用外部金鑰管理。外部金鑰管理是 ONTAP Select 9.19.1 唯一支援的金鑰管理。如需詳細資訊、請參閱link:reference_lic_ontap_features.html#licensed-ontap-features["預設授權的 ONTAP 功能"]。
+
--
ONTAP Select 不支援雲端金鑰管理器或 ONTAP Onboard Key Manager。
