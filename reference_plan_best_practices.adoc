...
NetApp建議使用 ESX 8.0 或更高版本，並使用 NVMe 磁碟作為託管系統磁碟的資料儲存。此配置可為NVRAM分區提供最佳效能。


NOTE: 在 ESX 8.0 或更高版本上安裝時，無論系統磁碟位於 SSD 或 NVMe 磁碟上， ONTAP Select都會使用 vNVMEe 驅動程式。這會將虛擬機器硬體等級設為 13，與 ESX 8.0 及更高版本相容。
