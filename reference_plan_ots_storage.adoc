...
NOTE: 本文概述了 ESXi 和 KVM 主機的外部儲存支援資訊。如需更多資訊,請參閱 link:reference_plan_ots_vnas.html["VMware ESXi 需求"] 和 link:reference_plan_ots_kvm.html["KVM 需求"]。
...
* VMware ESXi 8.0 GA（內部版本 20513097）或更高版本
...
NOTE: 如果存在硬體 RAID 控制器，請參閱 link:concept_stor_concepts_chars.html["深度儲存"] 部分以了解其他設定需求。
...
此外，軟體套件配置也有特定要求。請參閱 link:https://docs.netapp.com/us-en/ontap-select/kvm-host-configuration-and-preparation-checklist.html#prepare-linux-server["Linux 伺服器的準備"] 步驟以了解更多資訊。
