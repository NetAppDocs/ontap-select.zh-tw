...
. 在叢集右側選擇 image:icon_kebab.gif["選項"]，然後選擇 *Take Offline*。
+
如果離線選項無法使用、表示叢集已處於離線狀態。

. 在彈出視窗中選擇 *Yes* 以確認請求。
. 偶爾選取 *Refresh* 以確認叢集已離線。
. 若要使叢集重新上線，請選擇 image:icon_kebab.gif["選項"] 並選擇 *Bring Online* 。
. 偶爾選取 *Refresh* 以確認叢集是否處於線上狀態。
...
. 使用系統管理員帳戶登入部署公用程式Web使用者介面。
. 選取頁面頂端的 * 叢集 * 索引標籤、然後從清單中選取所需的叢集。
. 在叢集右側選擇 image:icon_kebab.gif["選項"]，然後選擇 *Delete*。
+
如果刪除選項無法使用、則叢集不會處於離線狀態。

. 偶爾選取 *Refresh* 以確認叢集已從清單中移除。
...
.步驟
. 使用系統管理員帳戶登入部署公用程式Web使用者介面。
. 選擇頁面左上角的 *Clusters* 標籤，然後從清單中選擇所需的叢集。
. 在頁面右側選擇 image:icon_kebab.gif["選項"]，然後選擇 *Cluster Refresh*。
