...
. 選取頁面頂端的 * 管理 * 索引標籤。
. 選擇 *Settings & AutoSupport*，然後選擇 image:icon_pencil.gif["編輯"]。
. 請根據您的環境提供適當的組態資料，然後選取 *Modify* 。
+
...
. 使用系統管理員帳戶登入部署公用程式Web使用者介面。
. 選取頁面頂端的 * 管理 * 索引標籤。
. 選取 *Events & Jobs*，然後選取 *Events*。
. （選用）選取 *Filter* 並建立篩選器以限制顯示的事件訊息。
...
. 使用系統管理員帳戶登入部署公用程式Web使用者介面。
. 選取頁面頂端的 * 管理 * 索引標籤。
. 選擇 *Settings & AutoSupport*，然後選擇 image:icon_kebab.gif["選項"]。
...
. （選用）在 *AutoSupport History* 下選擇正確的套件，然後選擇下載圖示，將 AutoSupport 檔案儲存至本機工作站。
